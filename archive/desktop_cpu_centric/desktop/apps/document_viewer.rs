...
impl DesktopApp for DocumentViewerApp {
    ...
    fn render(&mut self, ctx: &mut AppRenderContext) {
        ...
        // Account for: padding, scrollbar, and visual margin for borders/shadows
        let text_area_width = ctx.width - padding * 2.0 - scrollbar_width - 40.0;
        ...
        if self.lines.len() > self.visible_lines {
            ...
            let indicator_width = indicator.len() as f32 * char_width;
            let indicator_x = (ox + ctx.width - indicator_width - padding).max(ox + padding);
            ...
            // Simple scroll bar (within window bounds)
            let bar_height = ctx.height - padding * 2.0;
            let bar_y = padding + scroll_pct * (bar_height - 20.0);
            let bar_x = (ox + ctx.width - padding - char_width).max(ox + padding);
            text_renderer.add_text("|", bar_x, oy + bar_y, colors::DARK_GRAY);
        }
