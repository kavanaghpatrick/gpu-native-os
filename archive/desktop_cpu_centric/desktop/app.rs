...
pub trait DesktopApp: Send {
    ...
    /// Get pending launch requests from this app
    ///
    /// Apps can request launching other apps (e.g., terminal launching demos).
    /// Returns list of app names to launch. The desktop will clear these after processing.
...
impl AppRegistry {
    ...
    /// Find an existing app registration by name, or register a new one
    pub fn find_or_register(&mut self, name: &str, icon_index: u32) -> u32 {
        // Check if already registered
        for (id, app) in &self.registered {
            if app.name == name {
                return *id;
            }
        }
        // Register new
        self.register(name, icon_index)
    }

    /// Collect launch requests from all running apps
    pub fn collect_launch_requests(&mut self) -> Vec<String> {
        let mut requests = Vec::new();
        for instance in &mut self.instances {
            requests.extend(instance.app.take_launch_requests());
        }
        requests
    }
